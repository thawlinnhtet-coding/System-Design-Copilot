...
public class ReviewOutboxEventEntity {
	@Id private UUID id; @Column(name="review_job_id",nullable=false,updatable=false) private UUID reviewJobId; @Column(name="event_type",nullable=false,updatable=false) private String eventType; @Column(nullable=false) private String status; @Column(name="published_at") private Instant publishedAt; @Column(name="created_at",nullable=false,updatable=false) private Instant createdAt;
	...
	public UUID getId(){return id;} public UUID getReviewJobId(){return reviewJobId;} public boolean pending(){return "PENDING".equals(status);} public void published(Instant now){status="PUBLISHED";publishedAt=now;} public void requeue(){status="PENDING";publishedAt=null;}
}
