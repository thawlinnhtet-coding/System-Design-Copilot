...
public class ReviewRequestEntity {
	...
	public void processing(){status=ReviewStatus.PROCESSING;errorCode=null;} public void retrying(String code){status=ReviewStatus.RETRYING;errorCode=code;} public void complete(Instant now){status=ReviewStatus.COMPLETED;errorCode=null;completedAt=now;} public void fail(String code){status=ReviewStatus.FAILED;errorCode=code;} public void failFinal(String code){status=ReviewStatus.FAILED_FINAL;errorCode=code;} public void requeue(){status=ReviewStatus.QUEUED;errorCode=null;completedAt=null;}
}
