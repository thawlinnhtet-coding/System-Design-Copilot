...
public class ReviewJobEntity {
	...
	public boolean exhausted(){return attempts>=maxAttempts;} public void retry(String code,Instant now){status=ReviewJobStatus.RETRYING;lastFailureCode=code;leaseOwner=null;leaseExpiresAt=null;updatedAt=now;} public void deadLetter(String code,Instant now){status=ReviewJobStatus.DEAD_LETTERED;lastFailureCode=code;leaseOwner=null;leaseExpiresAt=null;updatedAt=now;} public void complete(Instant now){status=ReviewJobStatus.COMPLETED;leaseOwner=null;leaseExpiresAt=null;updatedAt=now;} public void requeue(Instant now){status=ReviewJobStatus.QUEUED;attempts=0;lastFailureCode=null;leaseOwner=null;leaseExpiresAt=null;updatedAt=now;}
}
