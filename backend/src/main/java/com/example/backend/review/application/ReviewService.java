...
public class ReviewService {
	...
	private ReviewDetails details(ReviewRequestEntity request){try{var completed=reviews.findByReviewRequestId(request.getId()).orElse(null);return new ReviewDetails(request.getId(),request.getWorkspaceId(),request.getRevisionId(),request.getStatus().name(),request.getErrorCode(),request.getCreatedAt(),request.getCompletedAt(),completed==null?null:mapper.readTree(completed.getOutput()));}catch(Exception exception){throw new IllegalStateException("Stored Review output is invalid",exception);}}
	...
	public record ReviewDetails(UUID id,UUID workspaceId,UUID revisionId,String status,String errorCode,Instant createdAt,Instant completedAt,tools.jackson.databind.JsonNode output) { }
}
