...
class ReviewControllerTests {
	...
	@Autowired MockMvc mvc; @Autowired ReviewService service; @Autowired ReviewJobRepository jobs; @Autowired ReviewRequestRepository requests; private final ObjectMapper json=new ObjectMapper();
	@Test void queuesImmutableRevisionAndCompletesDuplicateDeliveryOnlyOnce() throws Exception {var token=token("review_ok_"+System.nanoTime());var workspace=workspace(token,"Serve redirects reliably");document(token,workspace);consent(token);var response=mvc.perform(post("/api/v1/workspaces/{id}/reviews",workspace).header("Authorization",token).header("Idempotency-Key","review-key")).andExpect(status().isAccepted()).andExpect(jsonPath("$.status").value("QUEUED")).andReturn();var body=json.readTree(response.getResponse().getContentAsString());var requestId=UUID.fromString(body.path("id").asText());var revisionId=body.path("revisionId").asText();var job=jobs.findByReviewRequestId(requestId).orElseThrow();assertEquals(ReviewService.ProcessingOutcome.COMPLETED,service.process(job.getId(),"first"));assertEquals(ReviewService.ProcessingOutcome.IGNORED,service.process(job.getId(),"duplicate"));mvc.perform(get("/api/v1/workspaces/{workspace}/reviews/{request}",workspace,requestId).header("Authorization",token)).andExpect(status().isOk()).andExpect(jsonPath("$.status").value("COMPLETED")).andExpect(jsonPath("$.output.summary").value("Gateway design is clear."));mvc.perform(get("/api/v1/workspaces/{workspace}/reviews",workspace).header("Authorization",token)).andExpect(status().isOk()).andExpect(jsonPath("$[0].id").value(requestId.toString()));var failed=requests.findById(requestId).orElseThrow();failed.fail("ai_provider_unavailable");requests.saveAndFlush(failed);job.deadLetter("ai_provider_unavailable",Instant.now());jobs.saveAndFlush(job);mvc.perform(post("/api/v1/workspaces/{workspace}/reviews/{request}/retry",workspace,requestId).header("Authorization",token)).andExpect(status().isAccepted()).andExpect(jsonPath("$.status").value("QUEUED")).andExpect(jsonPath("$.revisionId").value(revisionId));}
