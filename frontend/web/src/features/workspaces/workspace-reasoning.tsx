...
function RequirementRow({ busy, item, onDelete, onSave }: { busy: boolean; item: Requirement; onDelete: () => void; onSave: (body: RequirementInput) => void }) {
  return <details className="group py-4"><summary aria-label={`Edit requirement: ${item.statement}`} className="flex cursor-pointer list-none items-start justify-between gap-4"><span className="flex min-w-0 items-start gap-2"><span aria-hidden="true" className="mt-0.5 text-signal transition-transform group-open:rotate-90">›</span><span><span className="font-mono text-[10px] uppercase tracking-[0.12em] text-signal">{item.kind === "NON_FUNCTIONAL" ? "Quality (non-functional)" : "Functional"} · {item.priority}</span><span className="mt-1 block text-sm font-semibold">{item.statement}</span></span></span><span className="flex shrink-0 items-center gap-3 text-xs text-text-muted"><span>{item.status}</span><span className="font-semibold text-signal group-open:hidden">Edit</span><span className="hidden font-semibold text-signal group-open:inline">Close</span></span></summary><form aria-label="Edit requirement" className="mt-4 grid gap-3" onSubmit={(event) => { event.preventDefault(); onSave(requirementBody(new FormData(event.currentTarget))); }}><input className="field sm:col-span-2" defaultValue={item.statement} name="statement" required /><select aria-label="Requirement kind" className="field" defaultValue={item.kind} name="kind"><option value="FUNCTIONAL">Functional</option><option value="NON_FUNCTIONAL">Quality (non-functional)</option></select><select aria-label="Requirement priority" className="field" defaultValue={item.priority} name="priority"><option value="MUST">Must</option><option value="SHOULD">Should</option><option value="COULD">Could</option></select><select aria-label="Requirement status" className="field" defaultValue={item.status} name="status"><option value="OPEN">Open</option><option value="SATISFIED">Satisfied</option><option value="DROPPED">Dropped</option></select><label className="grid gap-1 text-xs font-semibold text-text-muted">Order<input aria-label="Requirement order" className="field" defaultValue={item.orderIndex} min="0" name="orderIndex" type="number" /></label><label className="grid gap-1 text-xs font-semibold text-text-muted">Measurable target<input className="field" defaultValue={item.measurableTarget} name="measurableTarget" placeholder="Measurable target" /></label><label className="grid gap-1 text-xs font-semibold text-text-muted sm:col-span-2">Rationale or source<textarea className="field min-h-20" defaultValue={item.rationale} name="rationale" placeholder="Rationale or source" /></label><div className="flex gap-2 sm:col-span-2"><button className={primaryButton} disabled={busy} type="submit">Save Requirement</button><button className={`${buttonClass} text-danger hover:bg-danger/10`} disabled={busy} onClick={onDelete} type="button">Delete</button></div></form></details>;
}
...
function AssumptionRow({ busy, item, onDelete, onSave }: { busy: boolean; item: Assumption; onDelete: () => void; onSave: (body: AssumptionInput) => void }) {
  ...
  return <details className="group py-4"><summary aria-label={`Edit assumption: ${item.category}`} className="flex cursor-pointer list-none items-start justify-between gap-4"><span className="flex min-w-0 items-start gap-2"><span aria-hidden="true" className="mt-0.5 text-signal transition-transform group-open:rotate-90">›</span><span><span className="font-mono text-[10px] uppercase tracking-[0.12em] text-signal">{item.category}</span><span className="mt-1 block text-sm font-semibold">{item.quantitativeValue || "Unquantified"} {item.unit || ""}</span></span></span><span className="flex shrink-0 items-center gap-3 text-xs text-text-muted"><span>{item.confidence}</span><span className="font-semibold text-signal group-open:hidden">Edit</span><span className="hidden font-semibold text-signal group-open:inline">Close</span></span></summary><form aria-label="Edit assumption" className="mt-4 grid gap-3 sm:grid-cols-2" onSubmit={(event) => { event.preventDefault(); onSave(assumptionBody(new FormData(event.currentTarget))); }}><input aria-label="Assumption category" className="field" defaultValue={item.category} name="category" required /><input aria-label="Assumption value" className="field" defaultValue={item.quantitativeValue} name="quantitativeValue" placeholder="Value" /><select aria-label="Assumption unit" className="field" defaultValue={item.unit || ""} name="unit"><option value="">No unit</option>{legacyUnit ? <option value={legacyUnit}>{legacyUnit}</option> : null}{assumptionUnits.map((unit) => <option key={unit} value={unit}>{unit}</option>)}</select><select aria-label="Assumption confidence" className="field" defaultValue={item.confidence} name="confidence"><option value="LOW">Low confidence</option><option value="MEDIUM">Medium confidence</option><option value="HIGH">High confidence</option></select><label className="grid gap-1 text-xs font-semibold text-text-muted">Order<input aria-label="Assumption order" className="field" defaultValue={item.orderIndex} min="0" name="orderIndex" type="number" /></label><label className="grid gap-1 text-xs font-semibold text-text-muted sm:col-span-2">Rationale<textarea className="field min-h-20" defaultValue={item.rationale} name="rationale" placeholder="Rationale" /></label><div className="flex gap-2 sm:col-span-2"><button className={primaryButton} disabled={busy} type="submit">Save Assumption</button><button className={`${buttonClass} text-danger hover:bg-danger/10`} disabled={busy} onClick={onDelete} type="button">Delete</button></div></form></details>;
}
...
function QuestionRow({ busy, item, onDelete, onSave }: { busy: boolean; item: UnresolvedQuestion; onDelete: () => void; onSave: (body: QuestionInput) => void }) {
  return <details className="group py-4"><summary aria-label={`Edit question: ${item.question}`} className="flex cursor-pointer list-none items-start justify-between gap-4"><span className="flex min-w-0 items-start gap-2"><span aria-hidden="true" className="mt-0.5 text-signal transition-transform group-open:rotate-90">›</span><span><span className="font-mono text-[10px] uppercase tracking-[0.12em] text-signal">{item.status}</span><span className="mt-1 block text-sm font-semibold">{item.question}</span></span></span><span className="flex shrink-0 items-center gap-3 text-xs text-text-muted"><span>Open question</span><span className="font-semibold text-signal group-open:hidden">Edit</span><span className="hidden font-semibold text-signal group-open:inline">Close</span></span></summary><form aria-label="Edit question" className="mt-4 grid gap-3" onSubmit={(event) => { event.preventDefault(); const form = new FormData(event.currentTarget); onSave(questionBody(form)); }}><label className="grid gap-1 text-xs font-semibold text-text-muted">Question<input className="field" defaultValue={item.question} name="question" required /></label><label className="grid gap-1 text-xs font-semibold text-text-muted">Why does it matter?<textarea className="field min-h-20" defaultValue={item.whyItMatters} name="whyItMatters" required /></label><label className="grid gap-1 text-xs font-semibold text-text-muted">Status<select className="field" defaultValue={item.status} name="status"><option value="OPEN">Open</option><option value="RESOLVED">Resolved</option><option value="DEFERRED">Deferred</option></select></label><label className="grid gap-1 text-xs font-semibold text-text-muted">Order<input aria-label="Question order" className="field" defaultValue={item.orderIndex} min="0" name="orderIndex" type="number" /></label><label className="grid gap-1 text-xs font-semibold text-text-muted">Resolution notes<textarea className="field min-h-20" defaultValue={item.resolutionNotes} name="resolutionNotes" placeholder="Resolution notes" /></label><div className="flex gap-2"><button className={primaryButton} disabled={busy} type="submit">Save Question</button><button className={`${buttonClass} text-danger hover:bg-danger/10`} disabled={busy} onClick={onDelete} type="button">Delete</button></div></form></details>;
}

export function DecisionRow({ busy, item, onDelete, onSave }: { busy: boolean; item: Decision; onDelete: () => void; onSave: (body: DecisionInput) => void }) {
  return <details className="group py-4"><summary aria-label={`Edit decision: ${item.title}`} className="flex cursor-pointer list-none items-start justify-between gap-4"><span className="flex min-w-0 items-start gap-2"><span aria-hidden="true" className="mt-0.5 text-signal transition-transform group-open:rotate-90">›</span><span><span className="font-mono text-[10px] uppercase tracking-[0.12em] text-signal">{item.status}</span><span className="mt-1 block text-sm font-semibold">{item.title}</span><span className="mt-1 block text-sm text-text-muted">{item.chosenOption} — {item.rationale}</span></span></span><span className="flex shrink-0 items-center gap-3 text-xs text-text-muted"><span>Decision</span><span className="font-semibold text-signal group-open:hidden">Edit</span><span className="hidden font-semibold text-signal group-open:inline">Close</span></span></summary><form aria-label="Edit decision" className="mt-4 grid gap-3" onSubmit={(event) => { event.preventDefault(); const form = new FormData(event.currentTarget); onSave(decisionBody(form)); }}><input aria-label="Decision title" className="field" defaultValue={item.title} name="title" required /><input aria-label="Chosen option" className="field" defaultValue={item.chosenOption} name="chosenOption" required /><textarea aria-label="Decision rationale" className="field min-h-20" defaultValue={item.rationale} name="rationale" required /><select aria-label="Decision status" className="field" defaultValue={item.status} name="status"><option value="PROPOSED">Proposed</option><option value="ACCEPTED">Accepted</option><option value="SUPERSEDED">Superseded</option></select><label className="grid gap-1 text-xs font-semibold text-text-muted">Order<input aria-label="Decision order" className="field" defaultValue={item.orderIndex} min="0" name="orderIndex" type="number" /></label><div className="flex gap-2"><button className={primaryButton} disabled={busy} type="submit">Save Decision</button><button className={`${buttonClass} text-danger hover:bg-danger/10`} disabled={busy} onClick={onDelete} type="button">Delete</button></div></form></details>;
}
