    ...
    <main className="overflow-x-hidden bg-background text-foreground">
      ...
      <section className="bg-chrome-850 px-5 py-12 text-text-on-dark sm:px-8 lg:h-[760px] lg:px-16 lg:py-12" id="practice-demo">
        <div className="mx-auto max-w-[1312px]">
          ...
          <div aria-label="Practice stages" className="mt-7 grid border border-[#34403c] sm:grid-cols-4">
            {stages.map((stage, index) => <button aria-pressed={activeStage === index} className={`min-h-12 border-b border-[#34403c] px-4 text-left text-[13px] transition-colors last:border-b-0 sm:border-b-0 sm:border-r sm:last:border-r-0 ${activeStage === index ? "bg-background text-foreground" : "text-text-on-dark-secondary hover:bg-[#202624]"}`} key={stage.short} onClick={() => setActiveStage(index)} type="button"><span className={`mr-3 font-mono text-[11px] ${activeStage === index ? "text-signal" : ""}`}>{String(index + 1).padStart(2, "0")}</span>{stage.label}</button>)}
          </div>
...
function WorkspacePreview({ activeStage }: { activeStage: number }) {
  ...
  return <div className="mt-6 grid min-h-[510px] overflow-hidden rounded border border-line bg-surface text-foreground lg:h-[510px] lg:grid-cols-[116px_290px_520px_minmax(0,1fr)]">
